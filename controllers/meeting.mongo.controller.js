...
const createMeeting = async (req, res) => {
  try {
    ...
    // Use meet.ffmuc.net - a free Jitsi instance without authentication requirement
    // Alternative: jitsi.riot.im, meet.jit.si (requires auth), 8x8.vc (requires auth)
    const jitsiDomain = process.env.JITSI_DOMAIN || 'meet.ffmuc.net';
...
const getAllMeetings = async (req, res) => {
  try {
    const meetings = await Meeting.find().sort({ scheduled_time: -1 });
    res.json({ meetings });
  } catch (error) {
...
const deleteMeeting = async (req, res) => {
  try {
    ...
    console.log('Delete request - Meeting ID:', meeting_id, 'Faculty ID:', facultyId); // Debug log
    
    // Find meeting by meeting_id or _id
    ...
    // Check if faculty is the owner (unless admin)
    if (req.user.role !== 'admin' && meeting.faculty_id !== facultyId) {
      console.log('Unauthorized delete attempt:', facultyId, 'vs', meeting.faculty_id);
    ...
    console.log('Meeting deleted successfully:', meeting.meeting_id, 'by', facultyId); // Debug log
